...
def create_rating(body: RatingCreate, db: Session = Depends(get_db)):
    ...
    assessment = Assessment(**body.dict())
    db.add(assessment)

    # Update professional avg rating
    prof = db.query(Professional).filter(Professional.user_id == body.reviewee_id).first()
    if prof:
        result = db.query(func.avg(Assessment.rating), func.count(Assessment.id))\
            .filter(Assessment.reviewee_id == body.reviewee_id).first()
        prof.rating_avg   = round(float(result[0] or 0), 1)
        prof.rating_count = result[1]
